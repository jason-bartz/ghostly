...
/// Select `count` characters backward from the cursor by holding Shift and
/// pressing Left that many times. Best-effort: accurate in plain-text fields
/// where each "character" is one caret step. Wrong for rich-text editors,
/// fields with autocomplete popups, or fields where the user moved the cursor.
/// Called only from voice-edit replace; disabled for auto-submit / external-script.
fn select_backward(enigo: &mut Enigo, count: usize) -> Result<(), String> {
    if count == 0 {
        return Ok(());
    }
    // Cap selection to avoid runaway key spam on a bad count.
    let capped = count.min(10_000);
    enigo
...
pub fn paste(text: String, app_handle: AppHandle) -> Result<(), String> {
    paste_with_options(text, app_handle, PasteOptions::default())
}

#[derive(Debug, Clone, Default)]
pub struct PasteOptions {
    /// Override `append_trailing_space` from settings. `None` = inherit.
    pub append_trailing_space: Option<bool>,
    /// If `Some(n)` and > 0, select `n` characters backward before pasting.
    /// Used by the voice-editing loop to replace the previously pasted text.
    pub replace_prior_chars: Option<usize>,
    /// If true, skip the auto_submit keypress after paste regardless of settings.
    /// Voice-edit replacements never auto-submit — that would fire off a new chat message.
    pub suppress_auto_submit: bool,
}
...
pub fn paste_with_options(
    ...
    // Append trailing space if setting is enabled (may be overridden per-transcription)
    let append_space = options
        .append_trailing_space
        .unwrap_or(settings.append_trailing_space);
    ...
    // Voice-edit replace: select prior text before pasting so paste overwrites it.
    // Only safe for keystroke-based paste methods; external-script/None get skipped.
    if let Some(n) = options.replace_prior_chars {
        if n > 0
            && matches!(
                paste_method,
                PasteMethod::CtrlV
