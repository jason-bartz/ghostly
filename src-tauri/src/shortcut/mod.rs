...
pub fn change_binding(
    ...
    if let Err(e) = validate_shortcut_for_implementation(&binding, settings.keyboard_implementation)
    {
        ...
        // Re-register the original binding so the shortcut stays functional
        if let Err(re_err) = register_shortcut(&app, binding_to_modify.clone()) {
            error!(
                "change_binding: failed to re-register original binding '{}' after validation error: {}",
                binding_to_modify.current_binding, re_err
            );
        }
        return Err(e);
    }
    ...
    if let Err(e) = register_shortcut(&app, updated_binding.clone()) {
        ...
        if let Err(re_err) = register_shortcut(&app, binding_to_modify.clone()) {
            error!(
                "change_binding: failed to re-register original binding '{}' after failed change: {}",
                binding_to_modify.current_binding, re_err
            );
        }
        return Ok(BindingResponse {
...
fn register_all_shortcuts_for_implementation(
    ...
    for (id, default_binding) in &default_bindings {
        ...
        {
            let default_binding_str = default_binding.current_binding.clone();

            // If the default is also empty (e.g. edit_last_transcription has no default),
            // this is an optional unset binding — skip without reporting a reset.
            if default_binding_str.is_empty() {
                continue;
            }
            ...
            // Reset to default
            binding.current_binding = default_binding_str;
            current_settings
                .bindings
        ...
        // Skip bindings that have no shortcut configured
...
pub fn change_sound_theme_setting(app: AppHandle, theme: String) -> Result<(), String> {
    ...
    let parsed = match theme.as_str() {
        "subtle" => SoundTheme::Subtle,
        "marimba" => SoundTheme::Marimba,
        "pop" => SoundTheme::Pop,
        "custom" => SoundTheme::Custom,
        other => {
            warn!("Invalid sound theme '{}', defaulting to subtle", other);
            SoundTheme::Subtle
        }
    };
