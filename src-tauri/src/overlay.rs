...
/// Emits the transcription text to the overlay so the user can see what was captured
/// before it's pasted. The overlay must already be visible in transcribing/processing state.
pub fn emit_transcription_preview(app_handle: &AppHandle, text: &str) {
    if let Some(overlay_window) = app_handle.get_webview_window("recording_overlay") {
        let _ = overlay_window.emit("transcription-preview", text);
    }
}
